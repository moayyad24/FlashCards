...
class HomeRepoImpl extends DbHelper implements HomeRepo {
  ...
  Future<HomeModel> fetchHomeData() async {
    // Fetch all folders
    String foldersSql = '''
      SELECT f.folder_id, f.folder_title, f.folder_desc, f.folder_color, f.created_at,
    ...
    String setsSql = '''
      SELECT s.set_id, s.set_title, s.set_desc, set_color, set_icon, s.folder_id, s.created_at,
  ...
  @override
  Future<int> insertAnewSet(SetModel setModel) async {
    String sql = '''
  ...
  Future<int> insertAnewFolder(FolderModel folderModel) async {
    String sql = '''
    INSERT INTO folders(folder_title, folder_desc, folder_color)
    VALUES (?, ?, ?)
    ''';
    List<dynamic> arguments = [
      folderModel.title,
